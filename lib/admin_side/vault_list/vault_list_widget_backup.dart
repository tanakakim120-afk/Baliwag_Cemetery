...
class _VaultListWidgetState extends State<VaultListWidget> {
  ...
  // Helper: clear all loading dialogs
      ...
      builder: (context, snapshot) {
        ...
        List<VaultRecord> vaultRecordList = allVaultList.where((vault) {
          ...
          if (_model.textController?.text.isNotEmpty == true) {
            ...
            final vaultId = vault.vaultID?.id ?? '';
                  ...
                  children: [
                    ...
                    Expanded(
                      child: StreamBuilder<List<ContractRecord>>(
                        stream: Stream.fromFuture(_getAvailableNitches()),
                        builder: (context, snapshot) {
              ...
              content: Container(
                ...
                child: Column(
                  children: [
                    ...
                    Expanded(
                      child: StreamBuilder<List<ContractRecord>>(
                        stream: Stream.fromFuture(_getAvailableLots()),
                        builder: (context, snapshot) {
                          if (snapshot.connectionState ==
                              ConnectionState.waiting) {
                            return Center(
  ...
  // Method to execute nitche assignment
  void _executeNitcheAssignment(
      BuildContext context, VaultRecord vault, ContractRecord nitche) async {
    try {
      print('Starting nitche assignment...');
      print('Vault ID: ${vault.reference.id}');
      print('Nitche ID: ${nitche.reference.id}');
      print('Vault deceased name: ${vault.deceasedname}');
      print('Vault applicant name: ${vault.applicantname}');

      // Get current contract data to merge with vault data
      final currentContract = await nitche.reference.get();
      final currentData = currentContract.data() as Map<String, dynamic>;
      ...
      await nitche.reference.update(updatedData);

      print('Nitche contract updated successfully');
      ...
      // First try a simple dialog to test if context is working
      showDialog(
        context: context,
        barrierDismissible: false,
        builder: (BuildContext context) {
          return AlertDialog(
            title: Text('SUCCESS!'),
            content: Text('Nitche assignment completed successfully!'),
            actions: [
              TextButton(
                onPressed: () {
                  Navigator.of(context).pop();
      ...
      print('Error in nitche assignment: $e');
      // Show error message modal
      await showDialog(
        context: context,
        builder: (BuildContext dialogContext) => AlertDialog(
          title: const Text('Error'),
          content: Text('Error assigning nitche: ${e.toString()}'),
          actions: [
            TextButton(
              onPressed: () => Navigator.of(dialogContext).pop(),
              child: const Text('OK'),
  ...
  // Method to show success popup notification
  void _showSuccessPopup(BuildContext context, String title, String message) {
    print('_showSuccessPopup called with title: $title');
    print('_showSuccessPopup called with message: $message');
    showDialog(
      context: context,
      barrierDismissible: false,
      builder: (BuildContext context) {
        print('Building success popup dialog...');
        return AlertDialog(
          shape: RoundedRectangleBorder(
            borderRadius: BorderRadius.circular(16),
          ),
  ...
  void _downloadVaultPDF(List<VaultRecord> allVaultList) async {
    try {
      // Get the filtered vault records using the same logic as the main table
      final filteredVaults = allVaultList.where((vault) {
        // Apply text search filter
        if (_model.textController?.text.isNotEmpty == true) {
          final searchText = _model.textController!.text.toLowerCase().trim();
          final matchesSearch = vault.reference.id
                  .toLowerCase()
                  .contains(searchText) ||
              (vault.deceasedname.isNotEmpty &&
                  vault.deceasedname.last.toLowerCase().contains(searchText)) ||
              (vault.applicantname.isNotEmpty &&
                  vault.applicantname.last
                      .toLowerCase()
                      .contains(searchText)) ||
                    ...
                    if (formKey.currentState!.validate()) {
                      ...
                      if (confirmed == true) {
                        ...
                        try {
                          ...
                          // Refresh the vault list after dialog is closed
                          if (mounted) {
                            widgetSetState(() {
                              _refreshKey++;
                            });
          ...
          actions: [
            ...
            ElevatedButton(
              onPressed: () async {
                Navigator.of(context).pop();
                final downloadUrl =
                    await _proceedWithInvalidDocument(selectedFile);
                if (downloadUrl != null && onUploadComplete != null) {
                  onUploadComplete(downloadUrl);
                }
              },
              style: ElevatedButton.styleFrom(
  ...
  // Proceed with uploading invalid document (admin override)
  Future<String?> _proceedWithInvalidDocument(dynamic selectedFile) async {
    try {
      _showLoadingDialog('Uploading file to storage...');
      ...
      if (downloadUrl != null) {
        ...
        await _showMessageDialog(
          'Uploaded (Override)',
          'Document uploaded with admin override (unverified).',
        );
        return downloadUrl;
      }
      return null;
