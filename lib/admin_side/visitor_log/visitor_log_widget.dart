...
import 'visitor_log_model.dart';
export 'visitor_log_model.dart';

class VisitorLogWidget extends StatefulWidget {
  const VisitorLogWidget({super.key});

  static String routeName = 'visitorLog';
  static String routePath = '/visitorLog';

  @override
  State<VisitorLogWidget> createState() => _VisitorLogWidgetState();
}

class _VisitorLogWidgetState extends State<VisitorLogWidget> {
  late VisitorLogModel _model;
  ...
  void initState() {
    super.initState();
    _model = createModel(context, () => VisitorLogModel());
  ...
  Widget build(BuildContext context) {
    ...
    return StreamBuilder<List<VisitorlogRecord>>(
      stream: queryVisitorlogRecord(
        queryBuilder: (visitorlogRecord) =>
            visitorlogRecord.orderBy('timestamp', descending: true),
      ),
      builder: (context, snapshot) {
        ...
        List<VisitorlogRecord> allVisitorLogList = snapshot.data!;

        // Filter the data based on search text and date ranges
        List<VisitorlogRecord> visitorLogRecordList =
            allVisitorLogList.where((visitorLog) {
          // Apply text search filter
          if (_model.textController?.text.isNotEmpty == true) {
            final searchText = _model.textController!.text.toLowerCase();
            final id = visitorLog.reference.id.toLowerCase();
            final name = visitorLog.name.toLowerCase();
            final email = visitorLog.email.toLowerCase();
            final timestamp =
                visitorLog.timestamp?.toString().toLowerCase() ?? '';

            // Search in multiple fields including email
          ...
          if (_model.startDate != null || _model.endDate != null) {
            ...
            if (visitorLog.timestamp != null) {
              DateTime visitorLogTimestamp = visitorLog.timestamp!;
              ...
              if (startDate != null &&
                ...
                print(
                    'Warning: Start date is after end date: $startDate > $endDate');
                return false;
              }

              if (startDate != null &&
                  visitorLogTimestamp.isBefore(startDate)) {
                return false;
              }
              if (endDate != null && visitorLogTimestamp.isAfter(endDate)) {
                return false;
              }
              hasValidDate = true;
            }

            // If no valid dates found and filters are applied, exclude the visitor log
            if (!hasValidDate) {
              return false;
            }
          }
        ...
        print(
            'Visitor Log filtering: Total: ${allVisitorLogList.length}, Filtered: ${visitorLogRecordList.length}');
        if (_model.textController?.text.isNotEmpty == true) {
          print('Search term: "${_model.textController!.text}"');
        }
        if (_model.startDate != null || _model.endDate != null) {
          print(
              'Date range: ${_model.startDate?.toString()} to ${_model.endDate?.toString()}');
        }
                            ...
                            children: [
                              ...
                              Column(
                                ...
                                children: [
                                  ...
                                  Text(
                                    'View and manage all visitor activities',
                                    style: GoogleFonts.inter(
                                      fontSize: 16,
                                      color: const Color(0xFF6B7280),
                                      height: 1.4,
                              ...
                              Row(
                                children: [
                                    ...
                                    child: Container(
                                      ...
                                      child: TextFormField(
                                        ...
                                        decoration: InputDecoration(
                                          hintText:
                                              'Search visitors by visitor ID, name, email, timestamp...',
                                  ...
                                  Container(
                                    ...
                                    child: ElevatedButton.icon(
                                      onPressed: () async {
                                        await _downloadVisitorLogPDF(
                                            visitorLogData:
                                                visitorLogRecordList);
                                      },
                          ...
                          child: Container(
                            ...
                            child: Column(
                              children: [
                                  ...
                                  child: Row(
                                    children: [
                                      ...
                                      Container(
                                        ...
                                        child: Text(
                                          '${visitorLogRecordList.length} of ${allVisitorLogList.length} records',
                                          style: GoogleFonts.inter(
                                            fontSize: 14,
                                            fontWeight: FontWeight.w600,
                                            color: Color(0xFF18651C),
                                ...
                                // Enhanced Data Table
                                Expanded(
                                  child: FlutterFlowDataTable<VisitorlogRecord>(
                                    controller:
                                        _model.paginatedDataTableController1,
                                    data: visitorLogRecordList,
  ...
  // Method to download visitor log as PDF
  Future<void> _downloadVisitorLogPDF(
      {List<VisitorlogRecord>? visitorLogData}) async {
    try {
      // Show loading dialog
      showDialog(
        context: context,
      ...
      final List<VisitorlogRecord> currentData = visitorLogData ?? [];

      // Debug: Print PDF generation info
      print('PDF Generation: ${currentData.length} visitor log records');
      print('Search term: ${_model.textController?.text ?? "None"}');
      print(
          'Date range: ${_model.startDate?.toString() ?? "No start"} to ${_model.endDate?.toString() ?? "No end"}');
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
              'PDF report generated successfully! ${currentData.length} visitor log records included.'),
  ...
  Future<String> _generatePDFContent(
      List<VisitorlogRecord> visitorLogList) async {
...
<body>
    <div class="header">
        ...
        <div class="subtitle">Visitor Log Report</div>
    ...
    <div class="info">
        ...
        <div class="info-row">
            <span class="info-label">Total Visitor Log Records:</span>
            <span class="info-value">${visitorLogList.length}</span>
        </div>
        <div class="info-row">
...
''');
    ...
    // Table rows
    for (final visitorLog in visitorLogList) {
      content.writeln('''
            <tr>
                <td>${visitorLog.reference.id}</td>
                <td>${visitorLog.visitorid.isNotEmpty ? visitorLog.visitorid : 'N/A'}</td>
                <td>${visitorLog.name.isNotEmpty ? visitorLog.name : 'N/A'}</td>
                <td>${visitorLog.email.isNotEmpty ? visitorLog.email : 'N/A'}</td>
                <td>${visitorLog.timestamp?.toString() ?? 'N/A'}</td>
            </tr>
''');
    }
...
''');
  ...
  Future<void> _createAndDownloadPDF(
      String htmlContent, int visitorLogCount) async {
    try {
      ...
      final anchor = html.AnchorElement(href: url)
        ..setAttribute('download', 'visitor_log_report_$timestamp.html')
        ..click();
